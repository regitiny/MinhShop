...
import org.regitiny.minhshop.config.KafkaProperties;
...
class MinhShopKafkaResourceIT
{
  ...
  void setup()
  {
    ...
    MinhShopKafkaResource kafkaResource = new MinhShopKafkaResource(kafkaProperties);
  ...
  {
    restMockMvc
      .perform(post("/api/minh-shop-kafka/publish/topic-produce?message=value-produce"))
  ...
  void consumesMessages() throws Exception
  {
    ...
    MvcResult mvcResult = restMockMvc
      .perform(get("/api/minh-shop-kafka/consume?topic=topic-consume"))
      .andExpect(status().isOk())
